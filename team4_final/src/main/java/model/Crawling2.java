...
public class Crawling2 {
	...
	public ArrayList<ProductDTO> makeSampleProduct() {
		...
		try {
			...
			//ExpectedConditions.visibilityOfElementLocated : 특정 CSS 선택자로 지정한 요소가 페이지에 보일 때까지 대기
			//전체 00개 상품이 보일때까지 대기
			...
			for(int i=0; i < product.size(); i++) {		
				...
				try {
					...
					try {
						...
						productDTO.setModel_product_link(tag);//상품 링크
	...
	public ArrayList<GymDTO> makeSampleGym(){
		...
		try {
			...
			System.out.println("325 "+search.getText());
			...
			System.out.println("332 검색어 입력성공");
